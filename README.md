...
At the end of the day what your trying to achieve is something like this:
	...
	a) Display the HTML and CSS of the viewport so that First Contentful Paint happens as soon as possible.  Hide DOM object that require javascript to be full loaded. 

    b) After a certain period start loading external resources. 
	
	c) After loading resources begin to show DOM object that require all the javascript to be loaded and parsed by the browser. These resources are original hidden using Vanilla JS and are now made visible. (Fallback Experience)
...
This module has some options available for tweaking and retesting. 
	...
	- Defer JS Load For Milliseconds - The length of time to delay the load of external Javscript assets for (in milliseconds)
	- Defer CSS Load For Milliseconds - The length of time to delay the load of external CSS assets for (in milliseconds)
